...
class Entity(object):
    def __init__(
        ...
        if X is None:
            ...
            assert n_x_time == self.n_time, (
                "Exogenous covariates and Y should have same number of timestamps."
            )
            self.X = X
        ...
        if labels is None:
            ...
            assert labels.shape[-1] == self.n_time, (
                "Data and labels should have same number of timestamps."
            )
            self.labels = labels
...
class Dataset(object):
    def __init__(
        self, entities: List[Entity], name: str, verbose: bool = False
    ) -> "Dataset":
        """
        Parameters
        ----------
        ...
        # asserts
        assert all(
            [(entity.n_features == self.n_features) for entity in self.entities]
        ), "All entities must have same number of features!"
        assert all(
            [(entity.n_exogenous == self.n_exogenous) for entity in self.entities]
        ), "All entities must have same number of exogenous variables!"
