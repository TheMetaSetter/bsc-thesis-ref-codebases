...
class Loader(object):
    def __init__(
        ...
        if mask_position not in _VALID_MASK_POSITIONS:
            raise ValueError(
                f"mask_position must be one of {_VALID_MASK_POSITIONS}, {mask_position} was passed."
            )
    ...
    def _array_to_windows(self, X):
        ...
        if (self.window_size > 0) and (self.padding_type != "None"):
            self.padding_size = (
                self.window_step - (n_time - self.window_size) % self.window_step
            )
        ...
        windows = windows.unfold(
            dimension=-1, size=self.window_size, step=self.window_step
        )
    ...
    def __iter__(self):
        if self.shuffle:
            sample_idxs = np.random.choice(
                a=self.n_idxs, size=self.n_idxs, replace=self.sample_with_replace
            )
        else:
