...
def main(config_setting):
    ...
    for key, value in result_list.items():
        if key in final_eval_results:
            final_eval_results[key] = (
                f"{np.mean(result_list[key]):.4f}±{np.std(result_list[key]):.4f}"
            )
    dump_final_results(vars(config_setting), final_eval_results)

    print(
        f"----------------------{config_setting.dataset} Evaluation Results-----------------------"
    )
    print(
        f"{Color.CYAN}pc-a: {np.mean(result_list['pc_adjust']):.4f}{result_list['pc_adjust']}{Color.RESET}"
    )
    print(
        f"{Color.CYAN}rc-a: {np.mean(result_list['rc_adjust']):.4f}{result_list['rc_adjust']}{Color.RESET}"
    )
    print(
        f"{Color.CYAN}f1-a: {np.mean(result_list['f1_adjust']):.4f}{result_list['f1_adjust']}{Color.RESET}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--encoder_layers", type=int, default=1, help="The number of encoder layers"
    )
    parser.add_argument(
        "--branches_group_embedding",
        type=str,
        default="False_False",
        choices=["True_True", "True_False", "False_True", "False_False"],
        help="The parameter is used only when conv1d is employed in the encoder layer",
    )
    parser.add_argument(
        "--multiscale_kernel_size",
        nargs="+",
        type=int,
        default=[5],
        help="The parameter is used when conv1d is employed in the encoder layer",
    )
    parser.add_argument(
        "--multiscale_patch_size",
        nargs="+",
        type=int,
        default=[10, 20],
        help="The parameter is used when multi-attention is employed in the encoder layer",
    )
    parser.add_argument(
        "--branch1_networks",
        nargs="+",
    ...
    parser.add_argument(
        "--decoder_layers", type=int, default=1, help="The number of encoder layers"
    )
    parser.add_argument(
        "--decoder_group_embedding",
        type=str,
        default="False",
        choices=["True", "False"],
    )
    ...
    )  # for recon loss, max is better than mean
    ...
    parser.add_argument(
        ...
        type=str,
        default="optimal",
        choices=["preset", "optimal"],
    )
    parser.add_argument(
        "--anomaly_ratio",
        type=float,
        default=1.0,
        help="The parameter is used when threshold_setting is set as 'preset'",
    )
    parser.add_argument(
        "--run_times",
        type=int,
        default=5,
        help="The number of times to run for evaluating the result",
    )

    # debug parameters
    parser.add_argument(
        "--plot_data", type=str, default="False", choices=["True", "False"]
    )
    parser.add_argument(
        "--anomaly_only", type=str, default="False", choices=["True", "False"]
    )
    ...
    for frame_work in default_args.framework:
        ...
        if ("conv1d" not in config.branch2_networks) or (
            ...
            config.branches_group_embedding = (
                updated_group_embedding
                + "_"
                + config.branches_group_embedding.split("_")[1]
            )
