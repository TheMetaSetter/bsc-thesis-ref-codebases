    ...
    for i, methods_score in enumerate(tqdm(methods_keys)):
        ...
        for lag in tqdm(lag_range):
            ...
            R_AUC, R_AP, R_fpr, R_tpr, R_prec = grader.RangeAUC(
                labels=new_label,
                score=methods_scores[methods_score],
                window=slidingWindow,
                plot_ROC=True,
            )
            L, fpr, tpr = grader.metric_new(
                new_label, methods_scores[methods_score], plot_ROC=True
            )
            precision, recall, AP = grader.metric_PR(
                new_label, methods_scores[methods_score]
            )
            Y, Z, X, X_ap, W, Z_ap, avg_auc_3d, avg_ap_3d = generate_curve(
                new_label, methods_scores[methods_score], 2 * slidingWindow
            )
...
def compute_anomaly_acc_percentage(
    methods_scores, label, slidingWindow, methods_keys, pos_first_anom
):
    list_pos = []
...
def compute_anomaly_acc_pairwise(
    methods_scores, label, slidingWindow, method1, method2
):
    lag_range = list(range(-slidingWindow // 4, slidingWindow // 4, 5))
    methods_acc = {}
    method_key = [method1]
