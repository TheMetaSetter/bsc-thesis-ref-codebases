...
# -*- coding: utf-8 -*-
import math
from ._affiliation_zone import get_all_E_gt_func, affiliation_partition
...
def affiliation_recall_distance(Is=[(1, 2), (3, 4), (5, 6)], J=(2, 5.5)):
    ...
    Js = affiliation_partition(
        [J], E_gt_recall
    )  # partition of J depending of proximity with Is
    return sum(
        [integral_interval_distance(J[0], I) for I, J in zip(Is, Js)]
    ) / interval_length(J)
...
def affiliation_recall_proba(Is=[(1, 2), (3, 4), (5, 6)], J=(2, 5.5), E=(0, 8)):
    ...
    return sum(
        [integral_interval_probaCDF_recall(I, J[0], E) for I, J in zip(Is, Js)]
    ) / interval_length(J)
