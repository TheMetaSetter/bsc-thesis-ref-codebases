...
def complex_operator(net_layer, x):
    if not torch.is_complex(x):
        ...
        if isinstance(net_layer[0], nn.LSTM):
            return (
                torch.complex(net_layer[0](x.real)[0], net_layer[1](x.imag)[0])
                if isinstance(net_layer, nn.ModuleList)
                else torch.complex(net_layer(x.real)[0], net_layer(x.imag)[0])
            )
        else:
            return (
                torch.complex(net_layer[0](x.real), net_layer[1](x.imag))
                if isinstance(net_layer, nn.ModuleList)
                else torch.complex(net_layer(x.real), net_layer(x.imag))
            )
...
def complex_softmax(x, dim=-1):
    if not torch.is_complex(x):
        ...
        return torch.complex(
            torch.softmax(x.real, dim=dim), torch.softmax(x.imag, dim=dim)
        )
...
class Attention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask=None):
        ...
        attn_scores = complex_einsum(
            "nlhd,nshd->nhls", queries, keys
        )  # N x Head x L x L

        attn_weights = complex_dropout(
            self.dropout, complex_softmax(scale * attn_scores, dim=-1)
        )

        updated_values = complex_einsum(
            "nhls,nshd->nlhd", attn_weights, values
        )  # N x L x Head x d

        return updated_values.contiguous(), attn_weights.permute(0, 2, 1, 3).mean(
            dim=-2
        )
...
class AttentionLayer(nn.Module):
    def __init__(
        ...
        self.attn = Attention(
            window_size=w_size, mask_flag=mask_flag, scale=scale, dropout=dropout
        )
