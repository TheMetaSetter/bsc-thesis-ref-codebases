...
class TransformerVar(nn.Module):
    ...
    def __init__(
        self, config, n_heads=1, d_ff=128, dropout=0.3, activation="gelu", gain=0.02
    ):
        ...
        branch1_dim = (
            self.input_c if self.branch1_match_dimension == "none" else self.d_model
        )
        branch2_dim = (
            self.input_c if self.branch2_match_dimension == "none" else self.d_model
        )

        self.encoder_branch1 = InputEmbedding(
            in_dim=self.input_c,
            d_model=branch1_dim,
            n_window=self.win_size,
            dropout=dropout,
            ...
            init_type=self.embedding_init,
            device=self.device,
        )

        self.encoder_branch2 = InputEmbedding(
            in_dim=self.input_c,
            d_model=branch2_dim,
            n_window=self.win_size,
            dropout=dropout,
        ...
        self.mem_R, self.mem_I = create_memory_matrix(
            N=branch2_dim,
            L=self.win_size,
        ...
        branch1_out_dim = (
            self.output_c if self.branch1_match_dimension == "none" else self.d_model
        )
    ...
    def get_attn_score(self, query, key, scale=None):
        ...
        """
        scale = 1.0 / sqrt(query.size(-1)) if scale is None else 1.0 / scale
...
def create_memory_matrix(N, L, mem_type="sinusoid", option="option1"):
    with torch.no_grad():
        if mem_type == "sinusoid" or mem_type == "cosine_only":
            ...
            init_matrix_r = torch.nn.init.orthogonal_(
                torch.rand((N, L), dtype=torch.float)
            )
            init_matrix_i = torch.nn.init.orthogonal_(
                torch.rand((N, L), dtype=torch.float)
            )
            ...
            init_matrix_r = torch.nn.init.orthogonal_(
                torch.randn((N, L), dtype=torch.float)
            )
            init_matrix_i = torch.nn.init.orthogonal_(
                torch.randn((N, L), dtype=torch.float)
            )
