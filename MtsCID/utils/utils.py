...
def dump_final_results(params, eval_results):
    ...
    benchmark_results.extend(
        [eval_results[k] for k in metric_list if k in eval_results.keys()]
    )
    ...
    if os.path.exists(result_file_name):
        ...
        pd.DataFrame([benchmark_results], columns=df_title).to_csv(
            result_file_name, index=False, encoding="utf8"
        )
