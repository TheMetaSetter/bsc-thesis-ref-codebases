...
class WADISegLoader(Dataset):
    def __init__(self, root_path, win_size, step, mode="train"):
        ...
        test_data = pd.read_csv(
            os.path.join(root_path, "WADI_attackdataLABLE.csv"), header=1
        )
        ...
        labels = (
            test_data["Attack LABLE (1:No Attack, -1:Attack)"]
        ...
        train_nan_columns = {
            col for col in train_data.columns if train_data[col].isna().all()
        }
        test_nan_columns = {
            col for col in test_data.columns if test_data[col].isna().all()
        }
        common_nan_columns = list(train_nan_columns.intersection(test_nan_columns))

        train_data = train_data.drop(
            ["Row", "Date", "Time"] + common_nan_columns, axis=1
        )
...
def get_loader_segment(
    ...
    if mode == "train":
        ...
        train_loader = DataLoader(
            dataset=train_subset, batch_size=batch_size, shuffle=shuffle, drop_last=True
        )
        val_loader = DataLoader(
            dataset=val_subset, batch_size=batch_size, shuffle=shuffle
        )
