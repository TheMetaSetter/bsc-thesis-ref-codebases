...
class OneEarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        torch.save(
            model.state_dict(),
            os.path.join(path, str(self.dataset) + f"_checkpoint.pth"),
        )
...
class Solver(object):
    ...
    def __init__(self, config):
        ...
        self.train_loader, self.vali_loader = get_loader_segment(
            self.data_path,
    ...
    def model_init(self, config):
        ...
        if torch.cuda.is_available():
            self.model = torch.nn.DataParallel(
                self.model, device_ids=[0], output_device=0
            ).to(self.device)
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            attn_loss = (
                ...
                if attn is None
                else self.entropy_loss(attn) * self.alpha
            )
        ...
        return (
            ...
            np.average(valid_intra_loss_list),
        )
    ...
    def train(self):
        ...
        early_stopping = OneEarlyStopping(
            patience=self.patience, verbose=True, dataset_name=self.dataset
        )
        ...
        self.train_time_per_epoch = round(
            (time.time() - training_start_time) / (epoch + 1), 3
        )
    ...
    def test(self, params):
        ...
        if self.threshold_setting == "preset":
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                latent_score = torch.softmax(
                    gathering_loss(queries, mem_items) / self.temperature, dim=-1
                )
            ...
            train_attens_energy = np.concatenate(train_attens_energy, axis=0).reshape(
                -1
            )
            ...
            valid_attens_energy = np.concatenate(valid_attens_energy, axis=0).reshape(
                -1
            )
        ...
        if self.threshold_setting == "optimal":
            ...
            results = ts_metrics_enhanced(
                test_labels, point_adjustment(test_labels, test_energy), pred
            )
        else:
            ...
            _, _, _, _, _, _, threshold_setting_results = get_point_adjust_scores(
                gt, pred, test_energy, events
            )
        ...
        precision_adjust, recall_adjust, f_score_adjust = (
            results["pc_adjust"],
            results["rc_adjust"],
            results["f1_adjust"],
        )
        ...
        print("=" * 63)
        print(
            f"Dataset: {self.dataset} | Precision_adjusted: {precision_adjust:.4f} | Recall_adjusted: {recall_adjust:.4f} | f1_score_adjusted: {f_score_adjust:.4f} "
        )
...
def matthews_correlation_coefficient(TP, TN, FP, FN):
    ...
    denominator = (
        np.sqrt(TP + FP) * np.sqrt(TP + FN) * np.sqrt(TN + FP) * np.sqrt((TN + FN))
    )
